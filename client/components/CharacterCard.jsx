...
class ChracterCard extends React.Component {
  ...
  renderComics() {
    return this.props.character.comics.map(comic => {
      return <img src={`${comic.thumbnail.path}/portrait_fantastic.jpg`} />;
    });
  }
    ...
    return (
      <div>
        <MuiThemeProvider>
          <Card style={style}>
            <CardMedia
              overlay={<CardTitle title={this.props.character.name} />}
            >
              <img
        ...
        {this.props.character.comics.length > 0 ? (
          <div className="comics">{this.renderComics()}</div>
        ) : (
          <h1>No comics</h1>
        )}
      </div>
